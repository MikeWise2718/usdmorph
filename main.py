...
def morphLines(lines:list[str],usdprim:UsdPrim):
    global args
    ...
    for line in lines:
        (primKeyName,prim) = usdprim.primFromLine(lineidx)
        ...
        if isprim:
            ...
            if ofname!="":
                if ptype=="(ptype missing)":
                    ...
                    if args.subShader and usdprim.hasAttribute(primKeyName,"inputs:diffuseColor"):
                            nline = insertPtype(line,"Shader")
                            prim["ptype"] = "Shader"
                            nShaderChanges += 1
                            if verb>2:
                                print(str(lineidx),": "+Fore.MAGENTA+nline.rstrip()+" "+Fore.BLUE+primKeyName)
                    elif args.subShader and usdprim.hasBasename(primKeyName,"PreviewSurface"):
                            nline = insertPtype(line,"Shader")
                            prim["ptype"] = "Shader"
                            nShaderChanges += 1
                            if verb>2:
                                print(str(lineidx),": "+Fore.MAGENTA+nline.rstrip()+" "+Fore.BLUE+primKeyName)
                    elif args.subGeom and usdprim.hasBasename(primKeyName,"Reference"):
                            nline = insertPtype(line,"Skeleton")
                            prim["ptype"] = "Skeleton"
                            nSkelChanges += 1
                            if verb>2:
                                print(str(lineidx),": "+Fore.MAGENTA+nline.rstrip()+" "+Fore.BLUE+primKeyName)
                    elif args.subGeom and usdprim.hasBasenameSuffix(primKeyName,"_Clone_"):
                            nline = insertPtype(line,"SkelRoot")
                            prim["ptype"] = "SkelRoot"
                            nSkelChanges += 1
                            if verb>2:
                                print(str(lineidx),": "+Fore.MAGENTA+nline.rstrip()+" "+Fore.BLUE+primKeyName)
                    elif args.subXform and usdprim.hasAttribute(primKeyName,"transform"):
                            nline = insertPtype(line,"Xform")
                            prim["ptype"] = "Xform"
                            nXformChanges += 1
                            if verb>2:
                                print(str(lineidx),": "+Fore.MAGENTA+nline.rstrip()+" "+Fore.BLUE+primKeyName)
                elif ptype=="Mesh" and args.subSkelApi and usdprim.hasParentPrimOfPtype(prim,"SkelRoot"):
            ...
            if not primvarst_triggerd:
                if line.find("primvars:st")>=0:
                    if usdprim.hasParentPrimOfPtype(prim,"SkelRoot"):
    ...
    return olines,usdprim.dboutlines
